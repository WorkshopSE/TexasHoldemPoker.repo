...
namespace Poker.BE.Domain.Core
{
	...
	public sealed class GameCenter
	{
		...
		public void UpdateLeaguesLevels(int numOfUsers, ICollection<int> levels)
		{
			...
			List<int> SortedLevels = levels.ToList();
			SortedLevels.Sort();
			if (SortedLevels[1] > League.MAX_LEVEL || SortedLevels[SortedLevels.Count - 1] < League.MIN_LEVEL)
			{
				throw new UserLevelOutOfBoundException();
			}
			...
			for (int leagueIndex=6; leagueIndex>=6-LeaguesCount; leagueIndex--)
			{
				...
				if (leagueIndex == 6 - LeaguesCount)
				{
					LeaguesDictionary[leagueIndex].MinLevel = League.MIN_LEVEL;
				}
				//index out of bound check
				if (nextLevelIndex >= SortedLevels.Count)
				{
		...
		private int CalculateNumberOfLeagues(int numOfUsers)
		{
			if (numOfUsers == 1)
				return 1;
			return Math.Min(numOfUsers / 2, MAX_NUM_OF_LEAGUES);
		}
		#endregion
